...
class Feedback:
    ...
    default_text_x_angle = "Enter the x angle"
    default_text_y_angle = "Enter the y angle"
    def __init__(self, master):
        self.master=master
        #Level 0 Configuration
        master.title("SPxY Observation Interface")
        master.iconbitmap(r"C:\\Users\\rayen\\OneDrive\\Bureau\\EST\\GUI design\\EST_ICON.ico")
        ...
        antenna_selection.add_command(label="SRT")
        ...
        validate_cmd = master.register(self.validate_numeric_input)
        ...
        latitude.bind('<FocusIn>', lambda event: clear_default_text_xyz(event, latitude, self.default_text_latitude))
        latitude.bind('<FocusOut>', lambda event: restore_default_text_xyz(event, latitude, self.default_text_latitude))

        longitude = ttk.Entry(self.command_frame, width = 20)
        longitude.insert(0, self.default_text_longitude)
        longitude.bind('<FocusIn>', lambda event: clear_default_text_xyz(event, longitude, self.default_text_longitude))
        longitude.bind('<FocusOut>', lambda event: restore_default_text_xyz(event, longitude, self.default_text_longitude))

        altitude = ttk.Entry(self.command_frame, width=20, validate="key", validatecommand=(validate_cmd, '%P'))
        altitude.insert(0, self.default_text_altitude)
        altitude.bind('<FocusIn>', lambda event: clear_default_text_xyz(event, altitude, self.default_text_altitude))
        altitude.bind('<FocusOut>', lambda event: restore_default_text_xyz(event, altitude, self.default_text_altitude))

        azimuth = ttk.Entry(self.command_frame, width = 25)
        azimuth.insert(0, self.default_text_azimuth)
            # Bind the function to the <FocusIn> event
        azimuth.bind('<FocusIn>', lambda event: clear_default_text_angles(event, azimuth, self.default_text_azimuth))
        azimuth.bind('<FocusOut>', lambda event: restore_default_text_angles(event, azimuth, self.default_text_azimuth))

        elevation = ttk.Entry(self.command_frame, width = 25)
        elevation.insert(0, self.default_text_elevation)
            # Bind the function to the <FocusIn> event
        elevation.bind('<FocusIn>', lambda event: clear_default_text_angles(event, elevation, self.default_text_elevation))
        elevation.bind('<FocusOut>', lambda event: restore_default_text_angles(event, elevation, self.default_text_elevation))

        x_angle = ttk.Entry(self.command_frame, width = 25)
        x_angle.insert(0, self.default_text_x_angle)
            # Bind the function to the <FocusIn> event
        x_angle.bind('<FocusIn>', lambda event: clear_default_text_angles_xy(event, x_angle, self.default_text_x_angle))
        x_angle.bind('<FocusOut>', lambda event: restore_default_text_angles_xy(event, x_angle, self.default_text_x_angle))

        y_angle = ttk.Entry(self.command_frame, width = 25)
        y_angle.insert(0, self.default_text_y_angle)
            # Bind the function to the <FocusIn> event
        y_angle.bind('<FocusIn>', lambda event: clear_default_text_angles_xy(event, y_angle, self.default_text_y_angle))
        y_angle.bind('<FocusOut>', lambda event: restore_default_text_angles_xy(event, y_angle, self.default_text_y_angle))

         # Function to clear default text on focus
        def clear_default_text_xyz(event, entry_widget, default_text):
        ...
        def restore_default_text_xyz(event, entry_widget, default_text):
            current_text = entry_widget.get()
            if not current_text:
        ...
        def clear_default_text_angles(event, entry_widget, default_text):
            current_text = entry_widget.get()
            if current_text == default_text:
                entry_widget.delete(0, 'end')
                disable_xyz()
        ...
        def restore_default_text_angles(event, entry_widget, default_text):
            current_text = entry_widget.get()
            if not current_text:
                entry_widget.insert(0, default_text)
                enable_xyz()
        ...
        def clear_default_text_angles_xy(event, entry_widget, default_text):
            current_text = entry_widget.get()
            if current_text == default_text:
                entry_widget.delete(0, 'end')
                disable_xyz()
        ...
        def restore_default_text_angles_xy(event, entry_widget, default_text):
            current_text = entry_widget.get()
            if not current_text:
                entry_widget.insert(0, default_text)
                enable_xyz()
        ...
        def disable_angles_xy():
            x_angle.config(state=DISABLED)
            y_angle.config(state=DISABLED)
        
        def enable_angles_xy():
            x_angle.config(state=NORMAL)
            y_angle.config(state=NORMAL)

        Move_to_position = ttk.Button(self.command_frame, text = "Move to indicated position",command= self.moving_message)

        Stop = ttk.Button(self.command_frame,
                        text = "!! EMERGENCY STOP !!",
                        style="W.TButton",
                        command= self.emergency_message)
        ...
        ttk.Label(self.command_frame, text="x").grid(row=5,column=0,columnspan=2)
        x_angle.grid(row=6,column=0,columnspan=2,pady=(0,10))
        ttk.Label(self.command_frame, text="y").grid(row=5,column=1,columnspan=2)
        y_angle.grid(row=6,column=1,columnspan=2,pady=(0,10))
        Move_to_position.grid(row=7,columnspan=3,pady=(10,10))
        Stop.grid(row=8,columnspan=3,pady=10)
        ...
        self.data_display_frame.grid(row=0,column=0)
        self.graphic_display_frame.grid(row=0,column=1)

        #Level 3 data_display_frame
            # Add Text widget for information display
        self.general_info_display = Text(self.data_display_frame, width=100, height=3,font=('Arial',8))
        ...
        ttk.Label(self.data_display_frame, text="General information:").grid(row=0,sticky="sw")
        self.general_info_display.grid(row=1, padx=10, pady=5)
        ttk.Label(self.data_display_frame, text="General data:").grid(row=2,pady=(10,0),sticky="sw")
        self.general_data_display.grid(row=3, pady=5)
        ttk.Label(self.data_display_frame, text="Signal data:").grid(row=4,pady=(10,0),sticky="sw")
        self.signal_data_display.grid(row=5, pady=5)

        #Level 3 graphic_display_frame
        ttk.Label(self.graphic_display_frame, text="Camera display").grid(row=0,padx=100,sticky='s')
...
########################################      
    def update_general_info_display(self, filename):
        try:
            with open(filename, 'r') as file:
                file_content = file.read()
                self.general_info_display.delete(1.0, 'end')  # Clear previous content
                self.general_info_display.insert('end', file_content)
...
########################################
    def validate_numeric_input(self, new_value):
            if new_value =="" or new_value == self.default_text_altitude:
                return True  # Allow the default text temporarily
            try:
                float(new_value)
                return True
            except ValueError:
...
def main(): 
    ...
    feedback.update_general_info_display(r"C:\Users\rayen\OneDrive\Bureau\EST\GUI design\General information.rtf")
    root.mainloop()
